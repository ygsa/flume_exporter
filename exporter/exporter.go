...
func (e *Exporter) collectGaugeVec() bool {
        ...
        go func() {
                ...
                for _, url := range e.flumeMetricUrls {
                        ...
                        server := reg.FindStringSubmatch(url)[1]
                        for k, v := range m.Metrics[url] {
                                ...
                                if strings.HasPrefix(k, "SOURCE.") {
                                        e.processGaugeVecs(k, server, "SOURCE", sMetrics)
                                } else if strings.HasPrefix(k, "CHANNEL.") {
                                        delete(sMetrics, "Open")
                                        e.processGaugeVecs(k, server, "CHANNEL", sMetrics)
                                } else if strings.HasPrefix(k, "SINK.") {
                                        e.processGaugeVecs(k, server, "SINK", sMetrics)
                                }
                        }
                }
        }()
...
func (e *Exporter) processGaugeVecs(title string, server string, flumeType string, data map[string]interface{}) {
        ...
        for mName, mValue := range data {
                ...
                if gv == nil {
                        ...
                        gv.WithLabelValues(server, flumeType, name).Set(val)
                }
        }
}
